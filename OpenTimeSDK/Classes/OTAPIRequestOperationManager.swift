...
public class OTAPIRequestOperationManager: AFHTTPRequestOperationManager {
    ...
    required public init()
    {
        ...
        self.responseSerializer.acceptableContentTypes = ["text/html", "application/json"];
        self.setupHeaders();
    }
    ...
    public func apiResult(_ operation: AFHTTPRequestOperation!, error: NSError!, done: (_ response: OTAPIResponse)->Void)
    {
        if(error != nil && AFNetworkReachabilityManager.shared().isReachable == true) {
            ...
            let errroResponse: NSString = NSString.init(data: error.userInfo[AFNetworkingOperationFailingURLResponseDataErrorKey] as! Data, encoding: String.Encoding.utf8.rawValue)!;
            NSLog("%@", errroResponse);
